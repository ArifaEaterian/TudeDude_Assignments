...
int main()
{
    ...
    FOR(t, 1, T)
    {
        ...
        REP(j, M)
        {
            if (i == sa && j == sb || i == ea && j == eb)
                CanSee[i][j] = 0;
            else
                CanSee[i][j] = can_see(i, j, sa, sb) | can_see(i, j, ea, eb);
        }
        int t = Search();
        ...
        if (t == -1)
            ...
            cout << " The Shortest Path is " << t << " Steps long." << endl;
    }
    return 0;
}
...
int Search()
{
    ...
    while (Q.sz)
    {
        ...
        {
            int x = r + i, y = c + j;
            if (!ok(x, y) || Seen[x][y] != -1 || !CanSee[x][y])
                continue;
            if (!(ht >= Zc[x][y] && ht - Zc[x][y] <= 3 || Zc[x][y] >= ht && Zc[x][y] - ht <= 1))
                continue;
            Q.push(PII(x, y));
